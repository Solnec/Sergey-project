...
//1.Выходной параметр обязательно должен передаваться по ссылке, иначе создается копия, 
//которая в последствии и изменяется внутр метода и при выходе из скоупа удаляется
